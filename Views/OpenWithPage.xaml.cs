...
{
    public OpenWithViewModel VM { get; }

    public OpenWithPage()
    {
        InitializeComponent();
        VM = new OpenWithViewModel();
    ...
    private void Regedit_Click(object sender, RoutedEventArgs e) =>
        ExternalProgram.JumpRegEdit(@"HKEY_CLASSES_ROOT\Applications", null, false);

    private void DeleteButton_Click(object sender, RoutedEventArgs e)
    {
        if((sender as FrameworkElement)?.DataContext is OpenWithItemViewModel vm)
            _ = ConfirmAndDelete(vm);
    }

    /// <summary>删除前弹确认框（删除不可撤销），避免误操作。</summary>
    private async System.Threading.Tasks.Task ConfirmAndDelete(OpenWithItemViewModel vm)
    {
        ...
        if(!ok) return;
        if(vm.Delete(out string err)) VM.Items.Remove(vm);
        else VM.StatusText = AppStrings.Instance.Get("Status.DeleteFailed", err);
    }
}
